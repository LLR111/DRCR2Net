...
class StackBlocksDense(torch.nn.Module):
    ...
    def get_block_outputs(self, x, block_indices):
        outputs = []
...
class Xception(torch.nn.Module):
    ...
    def __init__(self, blocks, num_classes=None, global_pool=True, 
        ...
        feature1 += [Conv2dSame(3, 32, 3, stride=2),
                   Conv2dSame(32, 64, 3, stride=1)] #feature1
        ...
        if num_classes:
            layers += [torch.nn.Dropout2d(p=keep_prob, inplace=True),
                       torch.nn.Conv2d(blocks[-1].args[-1]['depth_list'][-1], 
                                       num_classes, 1)]
        self.feature1 = torch.nn.Sequential(*feature1)
        self._layers = torch.nn.Sequential(*layers)
        #print("layers_len: ",len(self._layers))
    ...
    def forward(self, x):
        ...
        """
        f1 = self.feature1(x)
        #f = self._layers[0].get_block_outputs(f1,[0,1,10,11])
        f = self._layers[0].get_block_outputs(f1,[0,1,10,12])
        return f1,f
...
def xception_block(scope,
    ...
    Args:
        ...
        unit_rate_list: A list of three integers, determining the unit rate in
            the corresponding backbone block.
...
def Xception41(num_classes=None,
    ...
    blocks = [
        ...
        xception_block('exit_flow/block2',
                       ...
                       num_units=1,
                       stride=1,
                       unit_rate_list=multi_grid), #feature5
    ]
...
def xception_41(num_classes=None,
                ...
                scope='xception_41',
                pretrained=True,
                checkpoint_path='./pretrain_xception/xception_41.pth'):
    """Xception-41 model."""
...
if __name__ == '__main__':
    path = "D:/Datasets/duhk&dut/test_data/dut/dut500-source/1.bmp"
    img = np.array(Image.open(path).convert('RGB')) / 255.
    img = np.transpose(img, (2, 0, 1))
    img = torch.tensor(img, dtype=torch.float32)
    img = img.unsqueeze(0)
    net = xception_41(num_classes=None, global_pool=False, pretrained=True)
    # state_dict = torch.load('./pretrain_xception/xception_41.pth')
